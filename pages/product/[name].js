...
const ItemView = (props) => {
  ...
  const { product } = props;
  const { price, img_url, name, description } = product;
  const {
    context: { addToCart },
  } = props;
  ...
  return (
    <>
      <CartLink />
      <Head>
        <title>Party ECommerce - {name}</title>
        <meta name="description" content={description} />
        <meta
          property="og:title"
          content={`Party ECommerce - ${name}`}
          key="title"
        />
      </Head>
      <div
      ...
      >
        <div className="w-full md:w-1/2 h-120 flex flex-1 bg-light hover:bg-light-200">
          <div className="py-16 p10 flex flex-1 justify-center items-center">
            <Image src={img_url} alt="Inventory item" className="max-h-full" />
          </div>
        </div>
